    ...
    <div className="min-h-screen px-5 pb-36 pt-10" style={{ backgroundColor: '#0F1120' }}>
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-[13px] mt-0.5" style={{ color: '#8B8BA7' }}>One task at a time. What matters most?</p>
        </div>
        <MochiAvatar size={44} />
      </motion.div>
      <div className="space-y-5">
            ...
            <div className="space-y-2">
              {modeOptions.map((m, i) => (
                <motion.button
                  ...
                  style={{
                    backgroundColor: selectedMode === i ? 'rgba(123,114,255,0.15)' : '#252840',
                    borderWidth: selectedMode === i ? 1.5 : 1,
                    borderStyle: 'solid',
                    borderColor: selectedMode === i ? '#7B72FF' : 'rgba(255,255,255,0.06)',
                  }}
                >
                  <div className="flex items-center gap-2.5">
                    <span className="text-[20px]">{m.emoji}</span>
                    <div>
                      <p className="text-[15px] font-semibold" style={{ color: selectedMode === i ? '#7B72FF' : '#E8E8F0' }}>{m.title}</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { content: <><span className="text-[20px] font-bold" style={{ color: '#E8E8F0' }}>47</span><span className="text-[11px] ml-1" style={{ color: '#8B8BA7' }}>done</span><span className="text-[11px] mx-1" style={{ color: '#8B8BA7' }}>·</span><span className="text-[20px] font-bold" style={{ color: '#E8E8F0' }}>12.5h</span></>, sub: '⚡ 2,340 XP' },
            { content: <span className="text-[28px]">😄</span>, sub: 'Tap to update' },
            { content: <span className="text-[20px] font-bold" style={{ color: '#E8E8F0' }}>🔥 5</span>, sub: 'day streak' },
            { content: <BurnoutGauge score={34} />, sub: 'Burnout gauge' },
          ].map((card, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 12 }}
