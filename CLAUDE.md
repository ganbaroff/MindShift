...
- **Capacitor:** `capacitor.config.ts` at project root. `appId: com.v0laura.mindshift`. Config includes SplashScreen/StatusBar/Keyboard/LocalNotifications/PushNotifications. `package.json` scripts: `cap:sync`, `cap:ios`, `cap:android`, `cap:run:android`. Install step: `npm install @capacitor/core @capacitor/cli @capacitor/ios @capacitor/android` (not committed — user runs once).
