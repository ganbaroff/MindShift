...
5. **🟠 ZEUS `/event` ingest is gone — `sync-to-zeus.yml` delivers to a dead API.** (Diagnosed 2026-07-10.) The workflow itself is fixed (was red on every push since April: multi-line `GITHUB_OUTPUT` broke the `summary` output before curl ever ran). But the target is structurally dead: `vars.ZEUS_GATEWAY_URL` → `zeus-gateway-production.up.railway.app` is now the **Claw3D ZEUS WS gateway** (Railway project `modest-happiness`, deployed from `ganbaroff/Claw3D@main`), which serves **only** `/api/gateway/ws` (WS+JWT) and `/health*`, everything else behind a `studio_access` cookie — no `/event` route, no `GATEWAY_SECRET`. The original gateway that had `POST /event` + Bearer `GATEWAY_SECRET` (Railway project `zesty-art`) has a FAILED last deploy dated 2026-04-11 and its domain 404s. The workflow now runs green and emits a `::warning::` annotation on non-2xx so the no-op stays visible. **To restore for real:** either add an HTTP `/event` ingest to the Claw3D server (+ set its secret in Railway and update the MindShift `GATEWAY_SECRET` GitHub secret), or retire this workflow. CEO call — see `memory/ceo-tasks.md`.
